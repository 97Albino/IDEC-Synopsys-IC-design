proc getCurrentInstance_AIC {} {
	return [db::getCurrentRef]
}

proc getCurrentParam_AIC {} {
	return [db::getCurrentParam]
}

proc getCurrentParamValue_AIC_AIC {param_name} {
	return [db::getParamValue $param_name -of [getCurrentInstance_AIC]]
}
...
proc getLayerPosition_AIC {layer} {
	return [lsearch -exact [getLayers_AIC] "$layer"]
}

proc set_Array_Inst_Contact_Limits_AIC {} {
	return [list [list min_width max_width min_height max_height] [list 0 100 0 100] ]
}

proc getLimitsValueByName_AIC { value_name } {
	set names  [lindex [set_Array_Inst_Contact_Limits_AIC] 0]
	set values [lindex [set_Array_Inst_Contact_Limits_AIC] 1]
...
proc isLayer_AIC {layer} {
	set layers [getLayers_AIC]
	set result 0
	foreach key $layers {
		if {$key == $layer} {
			set result 1
...
proc is_variable_AIC { paramValue } {
...
proc setInstanceParamValue_AIC { param value inst} {
	set formval [db::getParamValue $param -of $inst]
	
	# param is a variable
	if { [is_variable_AIC $formval] || [is_variable_AIC $value] } {
		set old_val $formval
		set new_val $value
...
proc ArrayInstContact32 {} {
	set layer1             [getCurrentParamValue_AIC "layer1"]
	set layer2             [getCurrentParamValue_AIC "layer2"]
	set routeDir1          [getCurrentParamValue_AIC "routeDir1"]
	set routeDir2          [getCurrentParamValue_AIC "routeDir2"]
	set width              [getCurrentParamValue_AIC "width"]
	set height             [getCurrentParamValue_AIC "height"]
	set addLayers          [getCurrentParamValue_AIC "addLayers"]
	set ruleset            [getCurrentParamValue_AIC "ruleset"]

	set edited_param [getCurrentParam]
	
	if {$edited_param == "layer1"} {
		if {[isLayer_AIC $layer1]} {
			set position_layer_1 [getLayerPosition_AIC $layer1]
			set position_layer_2 [getLayerPosition_AIC $layer2]
			...
			if {$position_layer_1 < $position_layer_2} {
				...
				setInstanceParamValue_AIC "layer1" "DIFF" [getCurrentInstance_AIC]
				setInstanceParamValue_AIC "layer2" "M1"   [getCurrentInstance_AIC]
			}
		} else {
		...
		set max_value_of_width [getLimitsValueByName_AIC "max_${edited_param}"]
		set min_value_of_width [getLimitsValueByName_AIC "min_${edited_param}"]
		if {$width > $max_value_of_width} {
			...
			setInstanceParamValue_AIC "${edited_param}" $max_value_of_width [getCurrentInstance_AIC]
			...
			setInstanceParamValue_AIC "${edited_param}" $min_value_of_width [getCurrentInstance_AIC]
		}
	} elseif {$edited_param == "height"} {
		set max_value_of_height [getLimitsValueByName_AIC "max_${edited_param}"]
		set min_value_of_height [getLimitsValueByName_AIC "min_${edited_param}"]
		if {$height > $max_value_of_height} {
			puts "WARNING 0010> The value of \"${edited_param}\" \"${height}\" >\[=\] ${max_value_of_height} max value..."
			puts "\t\tResetting \"${edited_param}\"  to max value."
			setInstanceParamValue_AIC "${edited_param}" $max_value_of_height [getCurrentInstance_AIC]
		} elseif {$height < $min_value_of_height} {
			puts "WARNING 0010> The value of \"${edited_param}\" \"${height}\" <\[=\] ${min_value_of_height} min value..."
			puts "\t\tResetting \"${edited_param}\"  to min value."
			setInstanceParamValue_AIC "${edited_param}" $min_value_of_height [getCurrentInstance_AIC]
		}
	} elseif {$edited_param == "addLayers"} {
		if {[isLayer_AIC $addLayers]} {
			
		} else {
			puts "WARNING 0011> The value of \"${edited_param}\" \"${addLayers}\" is not a layer..."
			puts "\t\tResetting \"${edited_param}\"  to default value."
			setInstanceParamValue_AIC "${edited_param}" "" [getCurrentInstance_AIC]
		}
	} elseif {$edited_param == "ruleset"} {
		setInstanceParamValue_AIC "${edited_param}" "default" [getCurrentInstance_AIC]
	}
}
