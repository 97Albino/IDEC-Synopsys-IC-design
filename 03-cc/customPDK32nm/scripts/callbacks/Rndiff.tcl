...
proc getCurrentParamValue_RNDIFF {param_name} {
	...
	set val [db::getParamValue $param_name -of [getCurrentInstance]]
	if {$val == ""} {
		set val 0
		set val [check_for_min_or_max_RNDIFF $param_name $val]
	}
	return $val
}
...
proc set_Limits_RNDIFF {} {
	...
	return [list \
		...
		[list 1      500     1      500     1    $limit 0     1e100  0.1     $limit   0.112   $limit 0.372   $limit 0.1       $limit] \
	]
}
...
proc setInstanceParamValue_RNDIFF { param value inst} {
	...
	if {[IsLayoutEditor]} {
		# param is a variable
		if { [is_variable_RNDIFF $formval] || [is_variable_RNDIFF $value] } {
			set old_val $formval
			set new_val $value
...
proc check_for_min {mode lf rw fr} {
	set min_value_lf [getLimitsValueByName_RNDIFF "min_lf"]
		
	set max [expr round([expr $lf/($min_value_lf)])]
	if {$mode=="rw"} {
	
		return [expr round([expr $max/$fr])]
	
	} else {
	
		return [expr round([expr $max/$rw])]
...
proc get_dw_RNDIFF {resistance_type} {
	if {$resistance_type == "rnpoly"} {
		return [expr -0.047*1e-6]
	} elseif {$resistance_type == "rppoly"} {
		return [expr -0.06*1e-6]
	} elseif {$resistance_type == "rndiff"} {
...
proc get_um_RNDIFF {resistance_type} {
	if {$resistance_type == "rnpoly"} {
		return 0.9
	} elseif {$resistance_type == "rppoly"} {
		return 0.9
...
proc get_r {resistance_type l w fr rw} {
	...
	set um	   [get_um_RNDIFF $resistance_type]
	#return [check_grid_RNDIFF [expr $fr*$rw*($sheetR*($l-$dl) + 2*$rend) / ($w - $dw)] 0.0001]
	
	return [expr round((($sheetR*($l*$fr*$rw*$um-$dl))/($w*$um - $dw))/2)]
}
...
proc get_w {resistance_type l r fr rw} {
	...
	#puts " sheetR= $sheetR rend=$rend dl=$dl dw=$dw um=$um"
		
	return [check_grid_RNDIFF [check_for_min_or_max_RNDIFF "wf" [expr ($sheetR*($fr*$rw*$l*$um-$dl)) / (2*$r*$um) + $dw/$um]] 0.001]
}
...
proc get_l {resistance_type w r fr rw} {
	...
	set um	   [get_um_RNDIFF $resistance_type]
	 ...
	 return [check_grid_RNDIFF [check_for_min_or_max_RNDIFF "lf" [expr ((2*$r*($w*$um-$dw)) / $sheetR + $dl)/($fr*$rw*$um)]] 0.001]
}
...
proc Rndiff32 {} {
	...
	set r [to_user [getCurrentParamValue "r"]]
	...
	if {$entryMode == "r & w"} {
		if {$edited_param == "r"} {
			set lf [get_l $resistance_type $wf $r $fr $rw]
			#set lf [check_grid_RNDIFF [expr $l/($fr*$rw)] 0.005]
			set r [db::sciToEng [get_r $resistance_type $lf $wf $fr $rw]]
			set l [expr $lf * $fr * $rw]
			
		} elseif {$edited_param == "wf"} {
			set lf [get_l $resistance_type $wf $r $fr $rw]
			#set lf [check_grid_RNDIFF [expr $l/($fr*$rw)] 0.005]
			set wf [get_w $resistance_type $lf $r $fr $rw]
			set l [expr $lf * $fr * $rw]
			#set r [db::sciToEng [get_r $resistance_type $l $wf $fr $rw]]
		...
		if {$edited_param == "r"} {
			...
			#puts "lf=$lf fr=$fr rw=$rw"
			#puts "resistance_type = $resistance_type"
			#set r  [db::sciToEng [get_r $resistance_type $l $wf $fr $rw]]
			set wf [get_w $resistance_type $lf $r $fr $rw]
			set lf [get_l $resistance_type $wf $r $fr $rw]
			set l  [expr $lf * $fr * $rw]			
	...
	if {$edited_param == "rw"} {
			...
			set rw_max [check_for_min "rw" $l $rw $fr]
			#puts "--------------------------- rw_max = $rw_max"
			if {$rw>$rw_max} {
				 puts "Cant set Finger Rows $rw case Lenght is too small. Setting Finger Rows to it's max value ($rw_max)"
				 set rw $rw_max
		...
		set fr [check_for_min_or_max_RNDIFF "fr" $fr]
			set fr_max [check_for_min "fr" $l $rw $fr]
			#puts "--------------------------- rw_max = $rw_max"
			if {$fr>$fr_max} {
				 puts "Cant set Finger Column $fr case Lenght is too small. Setting Column to it's max value ($fr_max)"
				 set fr $fr_max
				 gi::setActiveWindow 0 -raise true
			}
			set lf [get_l $resistance_type $wf $r $fr $rw]
